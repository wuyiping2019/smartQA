...
from utils.ListAndList import _index_q_list_in_k_list
...
class DataMaking(object):
    def __init__(self, do_lower_case=True, max_seq_length=128):
        self.task_name = "NER"
        # 初始化 bert_token 工具
        self.bert_tokenizer = tokenization.FullTokenizer(vocab_file=Properties.vocab_file, do_lower_case=do_lower_case)
    ...
    def convert_single_example(self, text, labeling_list=[], TEST_MODE=False):
        ...
        token_label_list = config.label_list
...
if __name__ == '__main__':
    ...
    data_make = DataMaking(do_lower_case=True, max_seq_length=config.max_seq_length)
    data_make.input2output(INPUT_DATA_PATHS, OUTPUT_DIR)
