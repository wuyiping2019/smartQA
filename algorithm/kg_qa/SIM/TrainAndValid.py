...
def get_input_data(input_file, seq_length, batch_size, is_training=True):
    def parser(record):
        name_to_features = {
            ...
            "label_ids": tf.FixedLenFeature([config.num_labels], tf.int64),
        }
...
def main():
    ...
    token_labels = tf.placeholder(tf.int64, shape=[None, num_labels], name='token_labels')
